...
#define SAMPLE_DEVICE_NAME "怀谷" // The Device Name Characteristics in GAP
...
static const uint8_t spp_adv_data[23] = {
    ...
    0x0F, ESP_BLE_AD_TYPE_NAME_CMPL, 'H', 'e', 'l', 'l', 'o', 'B', 'u', 't', 't', 'f', 'l', 'y'};
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        {
            // ESP_LOGI(GATTS_TABLE_TAG, "%d", res);
            ...
            {
                // 添加速率限制，防止数据发送过快导致芯片重启
                uint32_t current_time = xTaskGetTickCount();
                if ((current_time - g_last_process_time) > pdMS_TO_TICKS(50)) // 限制为每50ms处理一次
                {
                    // 检查指针和长度的有效性
                    if (p_data != NULL && p_data->write.value != NULL && p_data->write.len > 0)
                    {
                        ...
                        {
                            // 释放之前分配的内存
                            if (g_received_data.data != NULL)
                            {
                                free(g_received_data.data);
                            }

                            // 分配新内存并复制数据
                            g_received_data.data = (uint8_t *)malloc(p_data->write.len);
                            if (g_received_data.data != NULL)
                            {
                                memcpy(g_received_data.data, p_data->write.value, p_data->write.len);
                                g_received_data.len = p_data->write.len;
                                g_received_data.updated = true;
                            }
                            else
                            {
                                g_received_data.len = 0;
